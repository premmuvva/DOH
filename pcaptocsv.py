...
def process_pcap_wrapper(args):
    try: 
        file_path, i = args
        output_file_name = 'output/3/df'+str(i)+'.npy'
        if os.path.exists(output_file_name):
            print("File exists skipping")
        df = pd.DataFrame(columns=columns)
        process_pcap(file_path, df)
        # df_ar.append(i)
        np.save(output_file_name, df)
        del df
...
def get_process_params(dir_path):
    ...
    sorted_file_names = [(file[0], file[2]) for file in sorted_file_list]
...
process("/home/x286t435/thesis/time-series/dataset/Benign1234", 'output/Benign.csv')
